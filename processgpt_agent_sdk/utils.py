...
from .integrations.llm_proxy import completion_text
from .integrations.storage import (
    upload_file_to_bucket as _upload_file_to_bucket_impl,
    upload_files_to_bucket as _upload_files_to_bucket_impl,
)
...
async def _llm_request(system: str, user: str) -> str:
    ...
    text = await completion_text(messages, temperature=0)
    return text.strip()


def _build_error_summary_fallback(err_text: str, meta_text: str) -> str:
    """LLM 장애 시에도 사용자에게 전달 가능한 5줄 고정 문구."""
    return "\n".join(
        [
            f"현재 작업 처리 중 오류가 발생했습니다 ({err_text}).",
            "요청한 작업은 정상 완료되지 않았을 가능성이 큽니다.",
            "잠시 후 다시 시도하거나 입력값 및 연결 상태를 확인해 주세요.",
            f"반복 발생 시 작업 컨텍스트를 함께 점검하세요 ({meta_text or '컨텍스트 없음'}).",
            "문제가 계속되면 운영 담당자 또는 지원 채널로 문의해 주세요.",
        ]
    )


def _build_feedback_summary_fallback(
    feedback_data: List[dict], content_data: dict
) -> str:
    """LLM 장애 시 최소한의 통합 피드백 텍스트를 생성."""
    parts: List[str] = []
    if feedback_data:
        parts.append(f"[피드백]\n{feedback_data}")
    if content_data:
        parts.append(f"[현재 결과물]\n{content_data}")
    if not parts:
        return "피드백 정보가 없어 기본 요청으로 처리합니다."
    return "\n\n".join(parts)
...
async def summarize_error_to_user(exc: Exception, meta: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.warning("⚠️ LLM 오류 요약 생성 실패, fallback 사용: %s", e)
        return _build_error_summary_fallback(err_text=err_text, meta_text=meta_text)
...
async def summarize_feedback(feedback_data: List[dict], content_data: dict = {}) -> str:
    ...
    try:
        ...
        logger.warning("⚠️ LLM 피드백 요약 생성 실패, fallback 사용: %s", e)
        return _build_feedback_summary_fallback(feedback_data, content_data)
...
async def upload_file_to_bucket(
    ...
    """Deprecated: processgpt_agent_sdk.integrations.storage 사용."""
    warnings.warn(
        "processgpt_agent_sdk.utils.upload_file_to_bucket 는 deprecated 입니다. "
        "processgpt_agent_sdk.integrations.storage.upload_file_to_bucket 를 사용하세요.",
        DeprecationWarning,
        stacklevel=2,
    )
    return await _upload_file_to_bucket_impl(
        file=file, file_name=file_name, proc_inst_id=proc_inst_id
    )
...
async def upload_files_to_bucket(
    ...
    """Deprecated: processgpt_agent_sdk.integrations.storage 사용."""
    warnings.warn(
        "processgpt_agent_sdk.utils.upload_files_to_bucket 는 deprecated 입니다. "
        "processgpt_agent_sdk.integrations.storage.upload_files_to_bucket 를 사용하세요.",
        DeprecationWarning,
        stacklevel=2,
    )
    return await _upload_files_to_bucket_impl(files=files, proc_inst_id=proc_inst_id)
